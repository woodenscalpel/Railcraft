...
public class RCSoundHandler {
        ...
        if (soundName != null && event.sound instanceof PositionedSoundRecord && soundName.contains("railcraft")) {
            ...
            if (world != null) {
                ...
                if (sound != null) {
                    ...
                    if (soundName.contains("dig")) newName = sound.getBreakSound();
                    else if (soundName.contains("place")) newName = sound.func_150496_b();
                    event.result = new PositionedSoundRecord(
                            new ResourceLocation(newName),
                            event.sound.getVolume(),
                            event.sound.getPitch() * sound.getPitch(),
                            x,
                            y,
                            z);
                }
    ...
    private SoundType getBlockSound(World world, int x, int y, int z) {
        ...
        if (block instanceof IBlockSoundProvider) return ((IBlockSoundProvider) block).getSound(world, x, y, z);
        else {
