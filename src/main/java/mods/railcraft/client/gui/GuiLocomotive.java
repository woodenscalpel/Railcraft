...
public abstract class GuiLocomotive extends EntityGui {
    ...
    public GuiLocomotive(InventoryPlayer inv, EntityLocomotive loco, ContainerLocomotive container, String typeTag,
            String guiName, int guiHeight, boolean hasIdleMode) {
    ...
    public void initGui() {
        ...
        running = new GuiToggleButtonSmall(
                0,
                0,
                h + ySize - 129,
                55,
                LocalizationPlugin.translate("railcraft.gui.locomotive.mode.running"),
                loco.clientMode == LocoMode.RUNNING);
        buttons.add(running);
        idle = new GuiToggleButtonSmall(
                1,
                0,
                h + ySize - 129,
                45,
                LocalizationPlugin.translate("railcraft.gui.locomotive.mode.idle"),
                loco.clientMode == LocoMode.IDLE);
        if (hasIdleMode) buttons.add(idle);
        shutdown = new GuiToggleButtonSmall(
                2,
                0,
                h + ySize - 129,
                55,
                LocalizationPlugin.translate("railcraft.gui.locomotive.mode.shutdown"),
    ...
    private void updateButtons() {
        ...
        lockButton.setToolTip(
                loco.isPrivate() ? privateToolTips
                        : loco.isSecure() ? lockedToolTips : lockButton.enabled ? unlockedToolTips : null);
        ...
        if (ownerName != null && !ownerName.equals(locoOwner)) {
            ...
            unlockedToolTips = ToolTip
                    .buildToolTip("railcraft.gui.locomotive.tip.button.unlocked", "{owner}=" + ownerName);
            privateToolTips = ToolTip
                    .buildToolTip("railcraft.gui.locomotive.tip.button.private", "{owner}=" + ownerName);
        }
