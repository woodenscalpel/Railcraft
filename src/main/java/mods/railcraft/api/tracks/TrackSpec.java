...
/**
 * Each type of Track has a single instance of TrackSpec that corresponds with it. Each Track block in the world has a
 * ITrackInstance that corresponds with it.
 *
 * Take note of the difference (similar to block classes and tile entities classes).
 *
 * TrackSpecs must be registered with the TrackRegistry in either the Pre-Init or Init Phases.
 *
 * Track ItemStacks can be acquired from the TrackSpec, but you are required to register a proper display name yourself
 * (during Post-Init).
 *
 * @see TrackRegistry
 * @see ITrackInstance
...
public final class TrackSpec {
     ...
     *
     * @param trackId       A unique identifier for the track type. 0-512 are reserved for Railcraft. Capped at
     *                      Short.MAX_VALUE
     * @param tag           A unique internal string identifier (ex. "railcraft:track.speed.transition")
     ...
     */
    public TrackSpec(short trackId, String tag, ITrackItemIconProvider iconProvider,
     ...
     */
    public TrackSpec(short trackId, String tag, ITrackItemIconProvider iconProvider,
            Class<? extends ITrackInstance> instanceClass, List<String> tooltip) {
