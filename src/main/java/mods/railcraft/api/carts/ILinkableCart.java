...
/**
 * This interface should be implemented by any minecart that wishes to change the default linkage behavior. It is NOT
 * required to be able to link a cart, it merely gives you more control over the process.
 * 
...
public interface ILinkableCart {
    ...
    /**
     * Gets the distance at which this cart can be linked. This is called on both carts and added together to determine
     * how close two carts need to be for a successful link. Default = LinkageManager.LINKAGE_DISTANCE
     * 
    ...
    /**
     * Gets the optimal distance between linked carts. This is called on both carts and added together to determine the
     * optimal rest distance between linked carts. The LinkageManager will attempt to maintain this distance between
     * linked carts at all times. Default = LinkageManager.OPTIMAL_DISTANCE
     * 
    ...
    /**
     * Return false if linked carts have no effect on the velocity of this cart. Use carefully, if you link two carts
     * that can't be adjusted, it will behave as if they are not linked.
     * 
