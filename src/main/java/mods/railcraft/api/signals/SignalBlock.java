...
public abstract class SignalBlock extends AbstractPair {
    ...
    private final Map<WorldCoordinate, TrackScanner.ScanResult> trackScans =
            new HashMap<WorldCoordinate, TrackScanner.ScanResult>();
    ...
    private SignalBlock getSignalAt(WorldCoordinate coord) {
        ...
        if (recv != null) return ((ISignalBlockTile) recv).getSignalBlock();
        return null;
    }
    ...
    public void log(Level level, String msg, Object... args) {
        if (msg != null) LogManager.getLogger("Railcraft").log(level, new MessageFormatMessage(msg, args));
    }

    private void printDebug(String msg, Object... args) {
        if (SignalTools.printSignalDebug) log(DEBUG_LEVEL, msg, args);
    }
    ...
    private void printDebugPair(String msg, TileEntity ot) {
        if (SignalTools.printSignalDebug)
            if (ot == null)
                ...
                log(
                        DEBUG_LEVEL,
                        msg + " source:[{0}, {1}, {2}] target:[{3}, {4}, {5}] target class:{6}",
                        tile.xCoord,
                        tile.yCoord,
                        tile.zCoord,
                        ot.xCoord,
    ...
    protected void saveNBT(NBTTagCompound data) {
        ...
        //            printDebug("Signal Block saved NBT. [{0}, {1}, {2}] [verified: {3}] [changedAspect: {4}] [data:
        // {5}]", tile.xCoord, tile.yCoord, tile.zCoord, isConsistent, changedAspect, test);
        printDebug(
                "Signal Block saved NBT. [{0}, {1}, {2}] [changedAspect: {3}] [data: {4}]",
                tile.xCoord, tile.yCoord, tile.zCoord, changedAspect, pairings);
    ...
    protected void loadNBT(NBTTagCompound data) {
        ...
        printDebug(
                "Signal Block loaded NBT. [{0}, {1}, {2}] [data: {3}]",
                tile.xCoord, tile.yCoord, tile.zCoord, pairings);
        //        }
    }
    ...
    public void clearPairing(WorldCoordinate other) {
        ...
        if (SignalTools.printSignalDebug) {
            ...
            if (block != null)
                log(
                        DEBUG_LEVEL,
                        "Signal Block target block [{0}, {1}, {2}] = {3}, {4}",
                        x,
                        y,
                        z,
                        block.getClass(),
                        block.getUnlocalizedName());
            else log(DEBUG_LEVEL, "Signal Block target block [{0}, {1}, {2}] = null", x, y, z);
            TileEntity t = tile.getWorldObj().getTileEntity(x, y, z);
            if (t != null)
                log(
                        DEBUG_LEVEL,
                        "Signal Block target tile [{0}, {1}, {2}] = {3}",
                        t.xCoord,
                        t.yCoord,
                        t.zCoord,
                        t.getClass());
            else log(DEBUG_LEVEL, "Signal Block target tile [{0}, {1}, {2}] = null", x, y, z);
        }
    ...
    private void clearSignalBlockPairing(WorldCoordinate other, String reason, Object... args) {
        ...
        if (other == null) clearPairings();
        else clearPairing(other);
    }
    ...
    public void cleanPairings() {
        if (!invalidPairings.isEmpty())
            printDebug(
                    "Signal Block pairs cleaned: source:[{0}, {1}, {2}] targets: {3}",
                    tile.xCoord, tile.yCoord, tile.zCoord, invalidPairings);
        super.cleanPairings();
    }

    //    @Override
    //    public void startPairing() {
    //        clearSignalBlockPairing("Signal Block pairing cleared in preparation to start a new pairing.  [{0}, {1},
    // {2}]", tile.xCoord, tile.yCoord, tile.zCoord);
    //        super.startPairing();
    //    }
    public boolean createSignalBlock(SignalBlock other) {
        ...
        TrackScanner.ScanResult scan = TrackScanner.scanStraightTrackSection(
                tile.getWorldObj(), myTrack.x, myTrack.y, myTrack.z, otherTrack.x, otherTrack.y, otherTrack.z);
    ...
    public SignalAspect determineAspect(WorldCoordinate otherCoord) {
        if (isWaitingForRetest() || isBeingPaired()) return SignalAspect.BLINK_YELLOW;
        if (!isPaired()) return SignalAspect.BLINK_RED;
        SignalAspect otherAspect = SignalAspect.GREEN;
        SignalBlock other = getSignalAt(otherCoord);
        if (other != null) otherAspect = other.getSignalAspectForPair(getCoords());
    ...
    private SignalAspect determineMyAspect(WorldCoordinate otherCoord) {
        ...
        for (EntityMinecart cart : carts) {
            ...
            if (Math.abs(cart.motionX) < 0.08 && Math.abs(cart.motionZ) < 0.08) return SignalAspect.RED;
                ...
                if (cartZ > myTrack.z + zOffset && cart.motionZ < 0) return SignalAspect.RED;
                else if (cartZ < myTrack.z + zOffset && cart.motionZ > 0) return SignalAspect.RED;
                else newAspect = SignalAspect.YELLOW;
            else if (cartX > myTrack.x + xOffset && cart.motionX < 0) return SignalAspect.RED;
            else if (cartX < myTrack.x + xOffset && cart.motionX > 0) return SignalAspect.RED;
            else newAspect = SignalAspect.YELLOW;
        }
        return newAspect;
    ...
    private TrackScanner.ScanResult getOrCreateTrackScan(WorldCoordinate otherTrack) {
        ...
        if (scan == null) {
            WorldCoordinate myTrack = getTrackLocation();
            scan = TrackScanner.scanStraightTrackSection(
                    tile.getWorldObj(), myTrack.x, myTrack.y, myTrack.z, otherTrack.x, otherTrack.y, otherTrack.z);
            trackScans.put(otherTrack, scan);
        }
        return scan;
    ...
    private WorldCoordinate getOtherTrackLocation(WorldCoordinate otherCoord) {
        ...
        if (other != null) {
            ...
            if (track != null) trackCache.put(otherCoord, track);
            return track;
        }
    ...
    private TrackValidationStatus isSignalBlockValid(WorldCoordinate other) {
        if (other == null) return new TrackValidationStatus(true, "UNVERIFIABLE_COORD_NULL");
        SignalBlock otherSignalBlock = getSignalAt(other);
        if (otherSignalBlock == null) return new TrackValidationStatus(true, "UNVERIFIABLE_OTHER_SIGNAL_NULL");
        Status trackStatus = getTrackStatus();
        if (trackStatus == Status.INVALID) return new TrackValidationStatus(false, "INVALID_MY_TRACK_NULL");
        Status otherTrackStatus = otherSignalBlock.getTrackStatus();
        if (otherTrackStatus == Status.INVALID) return new TrackValidationStatus(false, "INVALID_OTHER_TRACK_INVALID");
        WorldCoordinate otherTrack = trackCache.get(other);
        if (otherTrackStatus == Status.UNKNOWN) {
            if (otherTrack == null) return new TrackValidationStatus(true, "UNVERIFIABLE_OTHER_TRACK_UNKNOWN");
        } else {
            otherTrack = otherSignalBlock.getTrackLocation();
            if (otherTrack != null) trackCache.put(other, otherTrack);
        }
        if (otherTrack == null) return new TrackValidationStatus(true, "UNVERIFIABLE_OTHER_TRACK_NULL");
        WorldCoordinate myTrack = getTrackLocation();
        TrackScanner.ScanResult scan = TrackScanner.scanStraightTrackSection(
                tile.getWorldObj(), myTrack.x, myTrack.y, myTrack.z, otherTrack.x, otherTrack.y, otherTrack.z);
        trackScans.put(otherTrack, scan);
        if (scan.verdict == TrackScanner.ScanResult.Verdict.VALID) return new TrackValidationStatus(true, "VALID");
    ...
    public void tickServer() {
        ...
        if (update % SignalTools.signalUpdateInterval == 0) {
            ...
            if (prev != SignalAspect.BLINK_RED) changedAspect = true;
            updateSignalAspect();
            if (getSignalAspect() == SignalAspect.BLINK_RED && prev != SignalAspect.BLINK_RED)
                printDebug(
                        "Signal Block changed aspect to BLINK_RED: source:[{0}, {1}, {2}] pairs: {3}",
                        tile.xCoord, tile.yCoord, tile.zCoord, pairings);
        }
        if (update % VALIDATION_CHECK_INTERVAL == 0) {
            ...
            switch (trackStatus) {
                case INVALID:
                    clearSignalBlockPairing(
                            null,
                            "Signal Block dropped because no track was found near Signal. [{0}, {1}, {2}]",
                            tile.xCoord,
                            tile.yCoord,
                            tile.zCoord);
                    break;
                case VALID:
                    for (WorldCoordinate otherCoord : waitingForRetest) {
                        ...
                        if (!status.isValid)
                            clearSignalBlockPairing(
                                    otherCoord,
                                    "Signal Block dropped because track between Signals was invalid. source:[{0}, {1}, {2}] target:[{3}, {4}, {5}] reason:{6}",
                                    tile.xCoord,
                                    tile.yCoord,
                                    tile.zCoord,
                                    otherCoord.x,
                    ...
                    for (WorldCoordinate otherCoord : getPairs()) {
                        if (!isSignalBlockValid(otherCoord).isValid) waitingForRetest.add(otherCoord);
                    }
                    break;
            }
        }
    ...
    public WorldCoordinate getTrackLocation() {
        if (trackLocation == null) locateTrack();
    ...
    private Status testForTrack(int x, int y, int z) {
        ...
        for (int jj = -2; jj < 4; jj++) {
            if (!world.blockExists(x, y - jj, z)) return Status.UNKNOWN;
