...
public final class MinecartHooks implements IMinecartCollisionHandler {
    ...
    public static MinecartHooks getInstance() {
        if (instance == null) instance = new MinecartHooks();
        return instance;
    }
        ...
        if (event.action == PlayerInteractEvent.Action.RIGHT_CLICK_BLOCK && itemStack != null) {
            ...
            if (item != null && CartUtils.vanillaCartItemMap.containsKey(item)) {
                ...
                EntityMinecart placedCart = CartUtils.placeCart(
                        ...
                        event.y,
                        event.z);
                if (placedCart != null && !player.capabilities.isCreativeMode) itemStack.stackSize--;
            }
        }
    }
    ...
    public void onEntityCollision(EntityMinecart cart, Entity other) {
        ...
        if (link != null && (link == other || other == link.riddenByEntity)) return;
        ...
        if (isLiving
                ...
                && other.ridingEntity == null) {
            int mountPrevention = cart.getEntityData().getInteger("MountPrevention");
            if (mountPrevention <= 0) other.mountEntity(cart);
        }
    ...
    public AxisAlignedBB getBoundingBox(EntityMinecart cart) {
        if (cart == null || cart.isDead) return null;
        if (RailcraftConfig.areCartsSolid()) return cart.boundingBox;
        return null;
    }
    ...
    public void onMinecartUpdate(MinecartUpdateEvent event) {
        ...
        if (TrackTools.isRailBlock(block)) {
            cart.fallDistance = 0;
            if (cart.riddenByEntity != null) cart.riddenByEntity.fallDistance = 0;
            if (launched > 1) land(cart);
        } else if (launched == 1) {
            data.setInteger("Launched", 2);
            cart.setCanUseRail(true);
        } else if (launched > 1 && (cart.onGround || cart.isInsideOfMaterial(Material.circuits))) land(cart);
        ...
        if (data.getBoolean("HighSpeed"))
            if (CartUtils.cartVelocityIsLessThan(cart, TrackSpeed.SPEED_CUTOFF)) data.setBoolean("HighSpeed", false);
            else if (!TrackSpeed.isTrackHighSpeedCapable(cart.worldObj, x, y, z)) CartUtils.explodeCart(cart);
    ...
    public void onMinecartEntityCollision(MinecartCollisionEvent event) {
        ...
        if (other == cart.riddenByEntity) return;

        if (other instanceof EntityMinecart) LinkageManager.instance().tryAutoLink(cart, (EntityMinecart) other);
        ...
        if (MiscTools.getRand().nextFloat() < 0.001f) {
            ...
            if (carts.size() >= 12) primeToExplode(cart);
        }
    }
    ...
    private void testHighSpeedCollision(EntityMinecart cart, Entity other) {
        ...
        if (highSpeed) {
            if (other instanceof EntityMinecart && Train.areInSameTrain(cart, (EntityMinecart) other)) return;
            for (EntityMinecart c : Train.getTrain(cart)) {
                if (other == c.riddenByEntity) return;
            }
            ...
            if (RailcraftConfig.isEntityExcludedFromHighSpeedExplosions(other)) return;
    ...
    public void onMinecartInteract(MinecartInteractEvent event) {
        ...
        if (!CartTools.doesCartHaveOwner(cart)) CartTools.setCartOwner(cart, player);
