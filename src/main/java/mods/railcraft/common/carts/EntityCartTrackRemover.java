...
public class EntityCartTrackRemover extends CartMaintenanceBase {
    ...
    protected void func_145821_a(
        ...
        for (WorldCoordinate track : tracksBehind) {
            if (track.isEqual(worldObj.provider.dimensionId, trackX, trackY, trackZ)) continue;
            removeTrack(track);
        }
    ...
    private void removeTrack(WorldCoordinate track) {
        if (WorldPlugin.getDistanceSq(track, posX, posY, posZ) >= 9) tracksRemoved.add(track);
        else if (!TrackTools.isRailBlockAt(worldObj, track.x, track.y, track.z)) tracksRemoved.add(track);
