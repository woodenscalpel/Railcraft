...
public class TrainTransferHelper implements mods.railcraft.api.carts.ITrainTransferHelper {
    ...
    private ItemStack _pushStack(EntityMinecart requester, Iterable<EntityMinecart> carts, ItemStack stack) {
        for (EntityMinecart cart : carts) {
            if (canAcceptPushedItem(requester, cart, stack)) stack = InvTools.moveItemStack(stack, (IInventory) cart);
            if (stack == null || !canPassItemRequests(cart)) break;
        }
        return stack;
    }
    ...
    private ItemStack _pullStack(EntityMinecart requester, Iterable<EntityMinecart> carts, IStackFilter filter) {
        for (EntityMinecart cart : carts) {
            ...
            if (!canPassItemRequests(cart)) break;
        }
        return null;
    }
    ...
    private boolean canAcceptPushedItem(EntityMinecart requester, EntityMinecart cart, ItemStack stack) {
        ...
        if (cart instanceof IItemCart) return ((IItemCart) cart).canAcceptPushedItem(requester, stack);
        return true;
    }

    private boolean canProvidePulledItem(EntityMinecart requester, EntityMinecart cart, ItemStack stack) {
        if (!(cart instanceof IInventory)) return false;
        if (cart instanceof IItemCart) return ((IItemCart) cart).canProvidePulledItem(requester, stack);
        return true;
    }

    private boolean canPassItemRequests(EntityMinecart cart) {
        if (cart instanceof IItemCart) return ((IItemCart) cart).canPassItemRequests();
        if (cart instanceof IInventory) return ((IInventory) cart).getSizeInventory() >= NUM_SLOTS;
        return false;
    }
    ...
    private FluidStack _pushFluid(EntityMinecart requester, Iterable<EntityMinecart> carts, FluidStack fluidStack) {
        ...
        for (EntityMinecart cart : carts) {
            ...
            if (fluidStack.amount <= 0 || !canPassFluidRequests(cart, fluidStack.getFluid())) break;
        }
    ...
    private FluidStack _pullFluid(EntityMinecart requester, Iterable<EntityMinecart> carts, FluidStack fluidStack) {
        for (EntityMinecart cart : carts) {
            ...
            if (!canPassFluidRequests(cart, fluidStack.getFluid())) break;
        }
        return null;
    }
    ...
    private boolean canAcceptPushedFluid(EntityMinecart requester, EntityMinecart cart, Fluid fluid) {
        ...
        if (cart instanceof IFluidCart) return ((IFluidCart) cart).canAcceptPushedFluid(requester, fluid);
    ...
    private boolean canProvidePulledFluid(EntityMinecart requester, EntityMinecart cart, Fluid fluid) {
        if (!(cart instanceof IFluidHandler)) return false;
        if (cart instanceof IFluidCart) return ((IFluidCart) cart).canProvidePulledFluid(requester, fluid);
    ...
    private boolean canPassFluidRequests(EntityMinecart cart, Fluid fluid) {
        if (cart instanceof IFluidCart) return ((IFluidCart) cart).canPassFluidRequests(fluid);
        if (cart instanceof IFluidHandler) {
            if (hasMatchingTank((IFluidHandler) cart, fluid)) return true;
        }
        return false;
    }
    ...
    private boolean hasMatchingTank(IFluidHandler handler, Fluid fluid) {
        ...
        for (FluidTankInfo info : tankInfo) {
            if (info.capacity >= TANK_CAPACITY) {
                if (info.fluid == null || info.fluid.amount == 0 || info.fluid.getFluid() == fluid) return true;
            }
        }
        return false;
    }
