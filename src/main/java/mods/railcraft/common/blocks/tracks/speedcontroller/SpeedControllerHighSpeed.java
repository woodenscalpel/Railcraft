...
public class SpeedControllerHighSpeed extends SpeedController {
    ...
    public static SpeedController getInstance() {
        if (instance == null) instance = new SpeedControllerHighSpeed();
        return instance;
    }
    ...
    public float getMaxSpeed(ITrackInstance track, EntityMinecart cart) {
        Float speed = null;
        if (track instanceof TrackSpeed) speed = ((TrackSpeed) track).maxSpeed;
        if (speed == null) speed = speedForCurrentTrack(track);
        if (track instanceof TrackSpeed) ((TrackSpeed) track).maxSpeed = speed;
        return speed;
    }
