...
public class BoardingLockingProfile extends LockingProfile {
    ...
    public void onRelease(EntityMinecart cart) {
        ...
        if (meta == 0 || meta == 4 || meta == 5) if (isReversed()) cart.motionZ += boostZ;
        else cart.motionZ -= boostZ;
        else if (meta == 1 || meta == 2 || meta == 3) if (isReversed()) cart.motionX -= boostX;
        else cart.motionX += boostX;
    }

    private boolean isReversed() {
        return track.getProfileType() == TrackNextGenLocking.LockingProfileType.BOARDING_B
                || track.getProfileType() == TrackNextGenLocking.LockingProfileType.BOARDING_B_TRAIN;
    }
}
