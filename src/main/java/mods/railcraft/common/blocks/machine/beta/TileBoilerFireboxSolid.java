...
public class TileBoilerFireboxSolid extends TileBoilerFirebox implements INeedsFuel {
    ...
    public void updateEntity() {
        ...
        if (isMaster && clock % 4 == 0) needsFuel = !InvTools.numItemsMoreThan(invFuel, 64);
    ...
    public boolean isItemValidForSlot(int slot, ItemStack stack) {
        ...
        if (slot >= SLOT_BURN) return FuelPlugin.getBurnTime(stack) > 0;
        else if (slot == SLOT_LIQUID_INPUT) return Fluids.WATER.isContained(stack);
        return false;
    }
    ...
    public boolean needsFuel() {
        TileBoilerFireboxSolid mBlock = (TileBoilerFireboxSolid) getMasterBlock();
        if (mBlock != null) return mBlock.needsFuel;
        return false;
    }
