...
public class SaltpeterGenerator {
    ...
    private boolean canGen(World world, Random rand, int x, int z) {
        ...
        if (!BiomeDictionary.isBiomeOfType(biome, BiomeDictionary.Type.SANDY)) return false;
        if (!BiomeDictionary.isBiomeOfType(biome, BiomeDictionary.Type.DRY)) return false;
        if (biome.canSpawnLightningBolt()) return false;
