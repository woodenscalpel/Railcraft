...
public class SulfurGenerator {
    ...
    public static final EventType EVENT_TYPE = EnumHelper
            .addEnum(EventType.class, "SULFUR", new Class[0], new Object[0]);
    ...
    public void generate(OreGenEvent.Post event) {
        ...
        if (!TerrainGen.generateOre(world, rand, sulfur, worldX, worldZ, EVENT_TYPE)) return;

        BiomeGenBase biome = world.getBiomeGenForCoords(worldX + 16, worldZ + 16);
        if (BiomeDictionary.isBiomeOfType(biome, BiomeDictionary.Type.MOUNTAIN)) for (int i = 0; i < 90; i++) {
