...
public class TankManager extends ForwardingList<StandardTank> implements IFluidHandler, List<StandardTank> {
    ...
    public static IFluidHandler getTankFromTile(TileEntity tile) {
        ...
        if (tile instanceof IFluidHandler) tank = (IFluidHandler) tile;
        return tank;
    }
    ...
    public int fill(int tankIndex, FluidStack resource, boolean doFill) {
        if (tankIndex < 0 || tankIndex >= tanks.size() || resource == null) return 0;
    ...
    public FluidStack drain(ForgeDirection from, FluidStack resource, boolean doDrain) {
        for (StandardTank tank : tanks) {
            if (tankCanDrainFluid(tank, resource)) return tank.drain(resource.amount, doDrain);
        }
        return null;
    }
    ...
    public void outputLiquid(IFluidHandler[] outputs, int tankIndex, int amount) {
        for (int side = 0; side < 6; side++) {
            ...
            if (nearbyTank != null) outputLiquid(nearbyTank, ForgeDirection.getOrientation(side), tankIndex, amount);
        }
    }
    ...
    private boolean tankCanDrainFluid(StandardTank tank, FluidStack fluidStack) {
        if (fluidStack == null) return false;
        if (!Fluids.areEqual(tank.getFluidType(), fluidStack)) return false;
