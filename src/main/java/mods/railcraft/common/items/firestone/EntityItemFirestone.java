...
public class EntityItemFirestone extends EntityItemFireproof {
    ...
    public static void register() {
        EntityRegistry.registerModEntity(
                EntityItemFirestone.class,
                "ItemFirestone",
                EntityIDs.ENTITY_ITEM_FIRESTONE,
                Railcraft.getMod(),
                64,
                20,
                true);
    ...
    protected void setOnFireFromLava() {
        ...
        if (worldObj.getBlock(xHit, yHit, zHit).getMaterial() == Material.lava
                || worldObj.getBlock(xHit, yHit + 1, zHit).getMaterial() == Material.lava)
            for (int y = yHit + 1; y <= yHit + 10; y++) {
                if (worldObj.isAirBlock(xHit, y, zHit)
                        && worldObj.getBlock(xHit, y - 1, zHit).getMaterial() == Material.lava) {
                    ...
                    if (tile instanceof TileFirestoneRecharge) {
                        ...
                        if (firestone.hasDisplayName()) fireTile.setItemName(firestone.getDisplayName());
                        setDead();
                        return;
                    }
                }
