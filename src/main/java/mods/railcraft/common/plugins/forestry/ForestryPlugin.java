...
public class ForestryPlugin {
    ...
    public static ForestryPlugin instance() {
        if (instance == null) {
            if (isForestryInstalled()) instance = new ForestryPluginInstalled();
            else instance = new ForestryPlugin();
        }
        return instance;
    }

    public static boolean isForestryInstalled() {
        if (modLoaded == null) modLoaded = Loader.isModLoaded("Forestry");
        return modLoaded;
    }

    public static ItemStack getItem(String tag) {
        if (!isForestryInstalled()) return null;
    ...
    private static class ForestryPluginInstalled extends ForestryPlugin {
        ...
        public void registerBackpacks() {
            try {
                if (forestry.api.storage.BackpackManager.backpackInterface == null) return;
