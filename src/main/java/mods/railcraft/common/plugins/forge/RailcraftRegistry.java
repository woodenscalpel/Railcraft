 ...
 * user's settings in "railcraft.cfg", so the available items may vary from one installation to the next.
 * <p/>
 * Initialization of the registry will occur during the pre-init and init stages. It is strongly recommended you wait
 * until the post-init stage to reference the registry.
 *
...
public final class RailcraftRegistry {
     ...
     * <p/>
     * Generally item tags will correspond to the tags used in "railcraft.cfg", but there will be some exceptions.
     * <p/>
     * This function can and will return null for just about every item if the item is disabled via the configuration
     * files. You must test the return value for safety.
     * <p/>
     ...
     * <p/>
     * This should generally only be called by Railcraft itself while the mod is initializing during the pre-init and
     * init stages.
     *
